...
module.exports = (config, request, utility) => {
   describe('Get user account', () => {
      const account = {
         ...
         avatar() { return crypto.createHash('md5').update(this.email().toLowerCase()).digest('hex'); },
      ...
      context('Invalid', () => {
         it('expects 400 because of invalid nickname', () => request.user.getUserAccount(utility.string(6), account.token)
            .then((res) => {
